...
export default class LoginUsuario implements CasoDeUso<Entrada, Usuario> {
	constructor(
		...
		private provedorCript: ProvedorCriptografia
	) {}

	async executar(entrada: Entrada): Promise<Usuario> {
		const usuarioExistente = await this.repositorio.buscaPorEmail(entrada.email)
		if (!usuarioExistente) throw new Error(Erros.USUARIO_NAO_EXISTE)
		const mesmaSenha = this.provedorCript.comparar(entrada.senha, usuarioExistente.senha!)
